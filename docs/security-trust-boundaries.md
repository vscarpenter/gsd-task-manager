...
| Browser auth store to OS clipboard | `components/settings/sync-auth-token-row.tsx`, `lib/sync/pocketbase-client.ts` | Require an explicit Copy auth token click or keyboard action; invoke the clipboard write within that user gesture without awaiting a network refresh; read the current raw token and reject missing or locally expired credentials immediately before writing; keep tokens out of rendered DOM, React state, logs and toast messages; hide export on sign-out; report success only after the write resolves and use generic errors for denied or unavailable clipboard access | `tests/ui/sync-auth-token-row.test.tsx` (current/rotated tokens, missing/expired auth, sign-out, pending/failed/unavailable clipboard, no DOM/toast disclosure); `tests/e2e/sync-auth-token.spec.ts` (keyboard/native clipboard, rotation, expiry, denial, sign-out, no DOM/console disclosure) |
| Browser to feedback collection | `lib/feedback/`, `components/settings/use-feedback-form.ts`, `scripts/setup-pocketbase-feedback-collection.sh`, `docker/pb_hooks/feedback_controls.pb.js` | Send only on explicit user action with no token or stable identifier; disable public creation until one all-audience 30-per-60-second rule, private logs, hook marker, transactional 10,000-record quota, and 180-day server-time retention all verify; fail closed on drift; block the marker at the public proxy | `tests/data/feedback-payload.test.ts`, `tests/data/feedback-store.test.ts`, `tests/data/submit-feedback.test.ts`, `tests/pb/feedback-controls-pocketbase-adapter.test.ts`, `packages/mcp-server/src/__tests__/system/pocketbase-system.test.ts` |
