...
| Browser to PocketBase sync | `lib/sync/`, `scripts/setup-pocketbase-collections.sh` | Require authentication and owner-scoped collection rules; make owner immutable; validate remote records; use `client_updated_at` for LWW and server `updated` for pull cursors; preserve archive tombstones | `tests/data/sync/`, `tests/sync/`, `tests/data/security-hardening-scripts.test.ts` |
| PocketBase API to SQLite | `docker/pb_hooks/tasks_encryption.pb.js`, `docker/pb_hooks/encryption-core.js`, `docker/pb_migrations/1781000000_encrypt_existing_tasks.js` | Require `GSD_TASKS_ENC_KEY`; encrypt selected content fields before create/update and decrypt after reads; keep the backfill idempotent | `tests/pb/`, `scripts/verify-pb-encryption.sh` |
