...
echo "Testing pull with actual device ID from JWT"
echo "============================================="

RESPONSE=$(curl -s -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"deviceId":"9Sc6Q_f_mj_pjhRkDM-vZg","lastVectorClock":{},"sinceTimestamp":1,"limit":10}' \
...
if [ "$HAS_ERROR" != "none" ]; then
  ...
  if [ "$TASK_COUNT" -gt 0 ]; then
    echo ""
    echo "First task preview:"
    echo "$RESPONSE" | jq '.tasks[0] | {id, encryptedBlobLength: (.encryptedBlob | length), nonceLength: (.nonce | length)}'
  fi
fi
