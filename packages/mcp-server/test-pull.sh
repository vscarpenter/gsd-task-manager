...
echo "Testing pull with sinceTimestamp: 0"
curl -s -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"deviceId":"mcp-server","lastVectorClock":{},"sinceTimestamp":0,"limit":10}' \
  https://gsd-sync-worker-production.vscarpenter.workers.dev/api/sync/pull | jq '.'

echo ""
echo "Testing pull without sinceTimestamp"
curl -s -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"deviceId":"mcp-server","lastVectorClock":{},"limit":10}' \
  https://gsd-sync-worker-production.vscarpenter.workers.dev/api/sync/pull | jq '.'
