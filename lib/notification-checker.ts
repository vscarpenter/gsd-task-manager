...
class NotificationChecker {
	...
	private async checkTask(
		...
		const minutesUntil = TIME_UTILS.msToMinutes(
	 ...
	 */
	start(
		intervalMinutes: number = NOTIFICATION_TIMING.DEFAULT_CHECK_INTERVAL_MINUTES,
	): void {
		// Prevent multiple starts - only start if not already running
		if (this.intervalId) {
			return;
		}
...
export async function getDueSoonCount(): Promise<number> {
	...
	for (const task of tasks) {
		...
		if (
			minutesUntil <= notifyBefore &&
			minutesUntil > NOTIFICATION_TIMING.OVERDUE_NOTIFICATION_THRESHOLD
		) {
			count++;
		}
	}
