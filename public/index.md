...
For Claude Desktop and other MCP-aware assistants, install an exact reviewed
...
Use the owner-only artifact generated by the wizard. It binds the bearer token
to the exact local Node executable and package entrypoint; updating requires an
explicit exact-version reinstall and another setup run. Environment variables
