---
name: "source-command-triage-prs"
description: "Night-shift maintenance routine (cycle D). Triages failing checks on the agent fleet's own (Codex/*) open PRs — auto-fixing mechanical classes as fix PRs, escalating judgment ones, skipping the rest. Never merges."
---
...
4. **Deliver a fix PR.** Commit the verified fix to a fresh `Codex/fix-<branch>-<runid>` branch, push, and `gh pr create --base <failing-branch>` (the fix targets the failing branch, so it re-enters review + CI). Comment the fix PR link on the original PR. **Do not merge.**
