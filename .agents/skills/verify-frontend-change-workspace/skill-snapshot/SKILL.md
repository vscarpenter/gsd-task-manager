...
2. **Playwright headless** — `bun run test:e2e` (or a targeted spec) against the auto-started dev server. No interactive screenshots, but real behavior in a real engine. Trace/screenshot on failure.
...
Never present rung 3 as if it were rung 1. A plan is not a PASS.
...
Loop: dev | build · Browser: live | playwright | inspection-only
